...
func NewHold(self, happen entity.Leg) Event {
	return &hold{self: self, happen: happen}
}

func (e *hold) Topic() string {
	return ievent.CloudTopicCall
}

func (e *hold) Type() string {
	return ievent.CloudTypeCallHold
}

func (e *hold) Subject() string {
	return subjectKey + e.self.Call().ID
}

func (e *hold) EventSubject() string {
	return e.self.PartType + "/" + e.self.PartID
}

func (e *hold) TenantID() string {
	return e.self.Call().TenantID
}

func (e *hold) MakeData() []byte {
	var (
		call  = e.self.Call()
		party vo.Party
	)
	e.data.Event = ievent.CloudEventHold
